...
## 3.1 Interceptor — ดักจับ Request/Response

**Interceptor** เป็นคุณสมบัติที่ได้รับแรงบันดาลใจจาก Aspect-Oriented Programming (AOP) ใน NestJS เราใช้ Interceptor เพื่อแทรกการทำงาน (Logic) บางอย่างเข้าไป **ก่อน** ที่ฟังก์ชัน (Route Handler) จะทำงาน และ **หลัง** จากที่ฟังก์ชันทำงานเสร็จ (หรือดักจับผลลัพธ์)

**การนำไปใช้งานจริง:**
*   **Logging / Performance Tracking:** จับเวลาตั้งแต่ request เข้ามาจนกระทั่งทำงานเสร็จ
*   **Data Transformation:** แปลงข้อมูล Response ให้อยู่ในรูปแบบที่เป็นมาตรฐาน (เช่น เอาข้อมูลไปห่อด้วย `{ data: ... }` เสมอ)
*   **Caching:** ดัก Response ไว้ ถ้าเคยถูก Request มาแล้วก็เอาของเก่าที่แคชไว้ส่งกลับไปเลย ไม่ต้องไปทำงานใน Service ใหม่
*   **ClassSerializerInterceptor:** ที่เราใช้ซ่อนพาสเวิร์ด (Exclude) ก็คือ Interceptor รูปแบบหนึ่ง

---

## 3.2 Custom Decorator

NestJS อนุญาตให้เราสร้าง **Decorator** เป็นของตัวเอง (Custom Decorator) เพื่อให้โค้ดสะอาดและอ่านง่ายขึ้น โดยไม่ต้องเขียน Logic เดิมซ้ำๆ ในหลาย Controller

**การนำไปใช้งานจริง:**
*   **ดึงข้อมูลจาก Request:** เช่น แทนที่จะต้องเขียน `@Request() req` แล้วค่อยไปดึง `req.user` เราสามารถสร้าง `@User()` ขึ้นมาใช้เอง ทำให้ใน Controller เขียนสั้นๆ แค่ `@User() user: UserEntity` ได้เลย
*   **แปะ Metadata:** เช่น `@Roles('ADMIN')` เป็น Custom Decorator ที่ใช้แปะ Metadata ไว้ที่ Route เพื่อให้ RolesGuard มาอ่านค่าเอาไปใช้เช็คสิทธิ์

---

## 3.3 Event-Driven — EventEmitter

**Event-Driven Architecture** คือรูปแบบการเขียนโค้ดที่โมดูลต่างๆ ไม่ต้องเรียกใช้กันตรงๆ (ลด Coupling) แต่ใช้วิธีการ "ประกาศ (Emit)" และ "รับฟัง (Listen)" แทน

ใน NestJS เราใช้แพ็กเกจ `@nestjs/event-emitter`
**ตัวอย่างการใช้งาน:**
เมื่อมีคนสมัครสมาชิกสำเร็จ (User Service ทำงานเสร็จ) แทนที่ User Service จะต้องเรียก Mail Service มาส่งอีเมลตรงๆ มันจะใช้วิธี "ตะโกน" (Emit Event) ว่า `user.created`
แล้ว Mail Service ที่ตั้งหูฟังอยู่ (`@OnEvent('user.created')`) ก็จะรับหน้าที่ไปส่งอีเมลต้อนรับเอง 
วิธีนี้ทำให้ User Service ไม่จำเป็นต้องรู้จักกับ Mail Service เลย ทำให้ระบบแยกขาดจากกัน (Decoupled) ได้ดีเยี่ยม

---

## 3.4 Queue — Bull/BullMQ

**Queue (คิว)** คือการนำงานที่ใช้เวลาประมวลผลนาน (Heavy or Long-running Task) ไปต่อแถวรอทำใน Background (เบื้องหลัง) เพื่อไม่ให้ผู้ใช้งานต้องรอนาน (บล็อก Main Thread)

ใน NestJS นิยมใช้แพ็กเกจ `@nestjs/bull` (ซึ่งครอบไลบรารี Bull หรือ BullMQ อีกที) โดยมันจะใช้ Redis เป็นตัวจัดการคิว
**ตัวอย่างการใช้งาน:**
*   การส่งอีเมลจำนวนมาก
*   การประมวลผลหรือแปลงขนาดรูปภาพ / วิดีโอ
*   การออกรายงาน (Export Report) หรือประมวลผลข้อมูลใหญ่ๆ
ผู้ใช้กดขอรีพอร์ต API จะตอบกลับไปทันทีว่า "รับเรื่องแล้ว กำลังประมวลผล" แล้วงานจริงๆ จะถูกส่งลงคิวให้ Worker ไปรันแบบเงียบๆ

---

## 3.5 Caching — Redis

**Caching (การแคช)** คือการนำข้อมูลที่ถูกดึงบ่อยๆ ไปเก็บไว้ในหน่วยความจำ (RAM) ที่อ่านได้เร็วสุดๆ เช่น **Redis** เพื่อที่ครั้งต่อไปเวลามีคนขอข้อมูลเดิม ระบบจะได้ไม่ต้องไป Query Database ใหม่อีกครั้ง ช่วยลดภาระ DB และเพิ่มความเร็วแอปได้มหาศาล

ใน NestJS มีระบบ Cache Module เตรียมไว้ให้ (`@nestjs/cache-manager`) 
เราสามารถใช้ `@UseInterceptors(CacheInterceptor)` แปะไว้บน Controller เพื่อเปิดระบบแคชของ API เส้นนั้นได้ง่ายๆ หรือจะเข้าไปจัดการแคชเองใน Service (เช่น `cacheManager.get()`, `cacheManager.set()`) ก็ได้เช่นกัน

---

## 3.6 CQRS Pattern

**CQRS (Command Query Responsibility Segregation)** เป็น Architecture Pattern ขั้นสูง ที่แยกส่วนระหว่าง "การอ่านข้อมูล (Query)" และ "การอัปเดตข้อมูล (Command)" ออกจากกันอย่างชัดเจน

*   **Command:** จัดการการสร้าง ลบ อัปเดต (Write operations)
*   **Query:** จัดการการดึงข้อมูล (Read operations)
ในระบบที่ซับซ้อนมาก การแยก Read/Write ช่วยให้เราปรับจูนประสิทธิภาพได้อิสระ เช่น เราอาจจะมี Database สำหรับอ่านข้อมูลต่างหากที่เชื่อมต่อหลายตัว (Read Replicas) 
ใน NestJS มีแพ็กเกจ `@nestjs/cqrs` ไว้รองรับ Pattern นี้ โดยทำงานผ่านระบบ Event Bus และ Command Bus

---

## 3.7 Hexagonal Architecture

**Hexagonal Architecture** (หรือ Ports and Adapters) เป็นรูปแบบการจัดโครงสร้างแอปพลิเคชันที่เน้นให้ **Core Business Logic (Domain)** อยู่ตรงกลาง และไม่ยึดติดกับเทคโนโลยีภายนอกเลย (เช่น Database, Framework, HTTP)

*   ตรงกลาง (Domain) จะมีแค่ Business Logic ล้วนๆ (เช่น การคำนวณราคาสินค้า กฎของระบบ)
*   ส่วนข้างนอก (Infrastructure) เช่น Database (Prisma), HTTP (Controller) จะถูกต่อเข้ามาผ่าน Interface (Port)
**ประโยชน์หลัก:** ทำให้แอปไม่ยึดติดกับ Framework (Framework-agnostic) ทดสอบโค้ดได้ 100% โดยไม่ต้องพึ่ง Database จริง และสามารถเปลี่ยน Database หรือย้ายไปใช้ Framework อื่นในอนาคตได้ง่าย

---

## 3.8 Testing — Unit / Integration / E2E

การเขียนเทส (Testing) ถือเป็นหัวใจสำคัญของการเขียนโค้ดที่มีคุณภาพ โดยแบ่งเป็น 3 ระดับ:

1.  **Unit Testing:** ทดสอบจุดที่เล็กที่สุดของโค้ด เช่น ทดสอบ Service หรือฟังก์ชันเดียว โดยมักจะใช้การจำลอง (Mocking) สิ่งแวดล้อมภายนอก เช่น Mock Prisma Service เพื่อให้เทสผ่านโดยไม่ต้องต่อ Database (ใช้ `Jest` เป็นหลัก)
2.  **Integration Testing:** ทดสอบการทำงานร่วมกันของชิ้นส่วนต่างๆ เช่น ทดสอบว่า Service สามารถคุยกับ Database จริงๆ ได้อย่างถูกต้อง
3.  **E2E (End-to-End) Testing:** ทดสอบระบบตั้งแต่ต้นจนจบ จำลองพฤติกรรมของ Client ที่ยิง HTTP Request เข้ามาผ่าน Controller จนจบกระบวนการ (ใน NestJS มักใช้คู่กับ `Supertest`)

---

## 3.9 The Twelve-Factor App Methodology

**12-Factor App** คือปรัชญาหรือกฎ 12 ข้อที่ออกแบบโดย Heroku สำหรับการสร้างเว็บแอปพลิเคชันยุคใหม่ (Cloud-Native) ให้มีประสิทธิภาพ ขยายสเกลได้ง่าย และพร้อมสำหรับใช้งานจริง (Production-Ready) กฎเด่นๆ ที่โปรเจกต์นี้ใช้งาน ได้แก่:
*   **Config:** การแยกการตั้งค่า (เช่น Password, Port) ออกจากโค้ดไปไว้ในไฟล์ Environment (`.env`)
*   **Backing Services:** มอง Database, Redis, MinIO เป็นเพียงบริการเสริมที่เอามาเสียบปลั๊กเชื่อมต่อ (รองรับด้วย Hexagonal Architecture)
*   **Disposability:** การทำ Graceful Shutdown เพื่อให้แอปพลิเคชันปิดตัวเองอย่างปลอดภัยที่สุดโดยไม่ทำให้ข้อมูลพัง
*   **Logs:** ให้แอปพลิเคชันพ่น Log ออกมาเป็น Stream (stdout) โดยตรง

---

## 3.10 Security Hardening (ความปลอดภัยขั้นสูงสุด)

ในระดับ Production การป้องกันการโจมตีเป็นสิ่งสำคัญมาก โปรเจกต์นี้ป้องกันตนเองผ่าน 2 เครื่องมือหลัก:
1.  **Helmet (`helmet`):** ช่วยเพิ่ม Security Headers ใน HTTP Response โดยอัตโนมัติ เพื่อป้องกันการโจมตีผ่านช่องโหว่ของเบราว์เซอร์ เช่น XSS (Cross-Site Scripting) หรือ Clickjacking
2.  **Rate Limiting (`@nestjs/throttler`):** เป็นกลไกป้องกันการโจมตีแบบ Spam หรือ Brute-force โดยการจำกัดจำนวน Request ที่สามารถเรียกเข้ามาได้จาก IP เดียวในช่วงเวลาที่กำหนด (เช่น 100 ครั้งใน 1 นาที)

---

## 3.11 Database Driver Adapters & Connection Pools

เพื่อให้แอปพลิเคชันทำงานได้รวดเร็วและใช้ทรัพยากรน้อยลง โปรเจกต์นี้เปลี่ยนจากการใช้ Prisma Engine แบบดั้งเดิม (Rust) มาใช้ **Driver Adapters (`@prisma/adapter-pg`)** ร่วมกับ Library `pg` โดยตรง
*   **Connection Pool:** สร้างบ่อพักสำหรับการเชื่อมต่อ Database (Pool) ทำให้เมื่อมี Request เข้ามามหาศาล ระบบจะไม่ไปสร้าง Connection ใหม่จน Database ล่ม แต่จะหมุนเวียนใช้จากท่อที่มีจำกัดตามที่เราตั้งไว้ใน `.env` (`DB_POOL_MAX`) อย่างเป็นระเบียบ

---

## 3.12 Observability (ระบบสังเกตการณ์: Logs, Metrics, Health)

การที่ระบบสามารถรายงานความผิดปกติของตัวเองได้ (Observability) คือหัวใจสำคัญของงาน DevOps
*   **Structured Logging (`nestjs-pino`):** บันทึก Log ออกมาในรูปแบบ JSON ที่มีโครงสร้างชัดเจน ทำให้ระบบส่วนกลางอย่าง Elasticsearch หรือ Datadog นำไปวิเคราะห์ต่อได้ง่ายกว่าข้อความธรรมดา
*   **Metrics (`@willsoto/nestjs-prometheus`):** เปิด Endpoint `/metrics` สำหรับเก็บข้อมูลดิบและสถิติต่างๆ ในแอป (เช่น จำนวน Request, ระยะเวลาประมวลผล) ให้ Prometheus มาดึงไปพล็อตเป็นกราฟ
*   **Health Checks (`@nestjs/terminus`):** สร้าง Endpoint `/health` สำหรับให้ Kubernetes หรือ Docker Swarm เข้ามาเช็คว่าแอปยัง "หายใจ" อยู่ไหม โดยตรวจสอบทะลุไปถึง Database และ Network

---

## 3.13 Database Migration & Seeding

*   **Migration (`prisma migrate dev / deploy`):** คือระบบ Version Control ของฐานข้อมูล แทนที่จะบังคับเปลี่ยนโครงสร้างตารางตรงๆ ระบบจะสร้างไฟล์ SQL เก็บประวัติการเปลี่ยนแปลงเอาไว้ ช่วยให้สามารถย้อนกลับ (Rollback) หรือสร้าง Database บนเซิร์ฟเวอร์ใหม่ได้แม่นยำ
*   **Seeding (`prisma db seed`):** คือกระบวนการใส่ "ข้อมูลตั้งต้น" (Initial Data) เข้าไปใน Database อัตโนมัติหลังจากการทำ Migration เสร็จ (เช่น การสร้าง User ระดับ Admin อัตโนมัติ หรือตารางข้อมูลอ้างอิง)

---

## 3.14 Docker Multi-stage Builds (การแพ็กเกจระดับ Production)

การสร้าง Docker Image ด้วยเทคนิค **Multi-stage Build** แบ่งการทำงานออกเป็น 2 ชั้น:
1.  **Builder Stage:** โหลดทุกอย่างที่ต้องใช้ในการคอมไพล์โค้ด (เช่น TypeScript compiler, devDependencies)
2.  **Production Stage:** ก๊อปปี้มาเฉพาะไฟล์ `.js` ที่คอมไพล์เสร็จแล้วและติดตั้งเฉพาะ `dependencies` ที่จำเป็นในการรัน (`npm ci --omit=dev`)
เทคนิคนี้ช่วยลดขนาดไฟล์อิมเมจลงได้อย่างมหาศาล (หลักสิบถึงร้อย MB) ลดเวลา Deploy และช่วยลดช่องโหว่ด้านความปลอดภัย (Security Attack Surface) บน Production ลงได้มาก

---

## 3.15 Resiliency & Circuit Breaker (ความทนทานต่อระบบภายนอกล่ม)

เวลาที่เราต้องไปดึงข้อมูลจาก API ของชาวบ้าน (External APIs) ถ้า API นั้นล่มหรือค้าง มันอาจจะพาลทำให้แอปของเราค้างและล่มตามไปด้วย (Cascading Failure) โปรเจกต์นี้จึงมีการออกแบบ **Resiliency Pattern** เอาไว้:
*   **ServiceHttpClient:** เราสร้างคลาสห่อหุ้ม (Wrapper) สำหรับการยิง HTTP Request ออกไปข้างนอก 
*   **Circuit Breaker (`opossum`):** ทำหน้าที่เหมือน "สะพานไฟ" ถ้ามันจับได้ว่า API ปลายทางเริ่มล่มบ่อยๆ (เช่น Error เกิน 50% หรือรอนานเกิน 3 วินาที) มันจะ "สับคัตเอาต์ (Open)" และตัดการเชื่อมต่อทันที ทำให้คำสั่งถัดๆ ไปถูกปฏิเสธอย่างรวดเร็ว (Fail Fast) โดยไม่ปล่อยให้แอปเราต้องไปรอจนค้าง เมื่อเวลาผ่านไปสักพัก มันจะลองแหย่เพื่อเชื่อมต่อใหม่ (Half-Open) ถ้าปกติก็จะกลับมาทำงานต่อ (Closed)

---

## 3.16 Object Storage Integration (ระบบจัดการไฟล์ด้วย MinIO)

แทนที่จะเก็บไฟล์รูปหรือเอกสารลงใน Harddisk ของเซิร์ฟเวอร์โดยตรง (ซึ่งผิดหลัก 12-Factor App) เราเปลี่ยนมาใช้ระบบ Object Storage:
*   **MinIO:** ซอฟต์แวร์จำลองเซิร์ฟเวอร์เก็บไฟล์ที่ทำงานด้วยมาตรฐานเดียวกับ **AWS S3** แบบ 100% 
*   เราสร้าง `MinioStorageAdapter` ที่ทำตามสัญญา (Interface) `IFileStorageProvider` ตามหลักการของ Hexagonal Architecture ทำให้ในอนาคตถ้าต้องการย้ายระบบจาก MinIO ไปขึ้นคลาวด์ AWS S3 ของจริง เราแทบไม่ต้องแก้โค้ดเลยแม้แต่บรรทัดเดียว
